...
class Midolman(Application, HypervisorService):
    ...
    def configure(self, host_cfg, app_cfg):
        ...
        :return:
        """
        self.LOG.debug("Individual host configuration for [" + host_cfg.name + "]")
        ...
        if u'eth0' in host_cfg.interfaces:
            if len(host_cfg.interfaces[u'eth0'].ip_addresses) == 0:
                raise ObjectNotFoundException('No IP address assigned on "eth0" for host: ' + self.name)
    ...
    def print_config(self, indent=0):
        ...
        print ('    ' * (indent + 1)) + 'Is-Hypervisor: ' + str(self.is_hypervisor())
    ...
    def create_vm(self, name):
        ...
        new_host.configure_logging(debug=self.host.debug)
        new_host.create()
        new_host.boot()
        new_host.net_up()
        new_host.net_up()
    ...
    def connect_iface_to_port(self, vm_host, iface, port_id):
        ...
        proc = self.host.run_app_command('bind_port', self, [near_if_name, port_id])
