...
class ConfiguredHostPTMImpl(PhysicalTopologyManagerImpl):
    ...
    def configure(self, config_file, file_type='json'):
        ...
        for host_cfg in ptc.hosts.itervalues():
            ...
            self.LOG.debug('Configuring host: ' + host_cfg.name + ' with impl: ' + impl_cfg.impl)
            ...
            for a in h.applications:
                if isinstance(a, HypervisorService):
                    ...
                    if a.is_hypervisor():
                        self.LOG.debug('Adding host application to hypervisor list:' +
    ...
    def create_vm(self, ip, mac=None, gw_ip=None, requested_hv_host=None, requested_vm_name=None):
        ...
        :param requested_hv_host: str: Hypervisor to use, otherwise the least-loaded HV host is chosen.
        :param requested_vm_name: str: Name to use for the VM.  Otherwise one is generated.
        :return: Guest
        """
        self.LOG.debug("Provisioning VM with IP: " + str(ip) +
                       (' on host: ' + requested_hv_host if requested_hv_host else '') +
                       (' with name: ' + requested_vm_name if requested_vm_name else ''))
        start_hv_app = None
        start_hv_host = None
        ...
        if not start_hv_host or not start_hv_app:
            raise ObjectNotFoundException('No suitable hypervisor service application found to launch VM')
        ...
        self.LOG.debug("Starting VM with name: " + requested_vm_name + " and IP: " +
                       ...
                       ' using hypervisor service application: ' + start_hv_app.name)
    ...
    def ptm_host_app_control(self, app_cmd, host_json, app_json, arg_list):
        ...
        h.config_host_for_process_control(host_cfg_map)
        ...
        if app_cmd == 'start':
            ...
            if fn is not None:
                ...
                raise ArgMismatchException('Command implementation function not found on app class: ' +
