    ...
    def add_test_from_pkg_mod_cls_func(self, current_context, test_type_obj, test_func_name):
        ...
        if test_type_obj is not None:
            ...
            if (mod_file_name == '__init__.py' or mod_file_name == '__init__.pyc'):
                ...
                try:
                    for test_name, test in inspect.getmembers(mod, predicate=inspect.isclass):
                        if test_name != 'TestCase' and inspect.isclass(test) and issubclass(test, TestCase):
    ...
    def run_scenario(self, scenario, test_list):
        ...
        for test_class, func_set in test_list:
            ...
            if scenario in test_class.supported_scenarios():
                ...
                log_file_name = test_class_name + "-" + scenario_name + ".log"
                ...
                scenario_log.debug('TSM: Preparing test class: ' + test_class_name)
                ...
                if self.test_system == 'unittest':
                    ...
                    if func_set is None:
                        ...
                        suite = test_loader.loadTestsFromNames(func_set)

                    # Flatten the tests
                    running_suite = unittest.TestSuite(self.test_suite_to_flat_list(suite))

                for tc in running_suite:
                    if self.debug:
