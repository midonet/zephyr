...
def parse_midolman_version(mnv, tag_ver=''):
    ...
    return Version(
        *map(
            lambda ver: ver.lstrip('0')
            if ver != '0'
            else '0', major_minor_patch[0:3]),
        tag=tag, epoch=epoch)
    ...
    if get_linux_dist() == LINUX_UBUNTU:
        ...
        if full_ver_str == "":
            raise ArgMismatchException(
                'Midolman package not found.  '
                'Zephyr cannot run without Midolman.')
...
class ConfigMap(object):
    ...
    def get_config_map(
            ...
            config_json=(
                ...
                '/midonet_version_configuration.json')):
    ...
    @classmethod
    def get_configured_parameter(
            cls, param, version=None,
            config_json=(os.path.dirname(os.path.realpath(__file__)) +
                         '/midonet_version_configuration.json')):
        """
        Retrieve a parameter based on the actively installed verison
        of Midolman.  If Midolman is not installed, do not use this function
        as it will throw an exception.  Instead, get the map manually and
        check the keys with the 'get_config_map' function.
        :type param: str
        :type version: str
        ...
        if mn_version.major not in cls.major_version_config_map:
            raise ObjectNotFoundException(
                'No version configuration found for major version: ' +
                str(mn_version.major))

        major_version_params = cls.major_version_config_map[mn_version.major]

        # Only use minor params, if it is present, otherwise
        # assume major params will work for unlisted minor versions
        ...
        # Minor version config takes precedence and can override
        # major version config
        ...
        raise ObjectNotFoundException(
            "Param not found in either major or minor version config: " +
            param)
