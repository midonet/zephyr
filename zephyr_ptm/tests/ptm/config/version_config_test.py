...
CFG_FILE = ROOT_DIR + '/ptm/config/midonet_version_configuration.json'


class VersionConfigTest(unittest.TestCase):
    def test_version_map(self):
        self.assertFalse(
            version_config.ConfigMap.get_config_map(
                config_json=CFG_FILE)['1']['option_use_v2_stack'])
    ...
    def test_vars(self):
        print(version_config.ConfigMap.get_configured_parameter(
            'mn_version', config_json=CFG_FILE))
        print(version_config.ConfigMap.get_configured_parameter(
            'cmd_list_datapath', config_json=CFG_FILE))
        print(version_config.ConfigMap.get_configured_parameter(
            'option_config_mnconf', config_json=CFG_FILE))
        print(version_config.ConfigMap.get_configured_parameter(
            'option_use_v2_stack', config_json=CFG_FILE))
