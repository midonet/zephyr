...
class Cassandra(Application):
    ...
    def wait_for_process_start(self):
        # Wait a couple seconds for the process to start before
        # polling nodetool
        time.sleep(2)
    ...
    def control_start(self):
        self.cli.cmd(
            '/bin/bash -c "MAX_HEAP_SIZE=128M HEAP_NEWSIZE=64M '
...
class CassandraFileConfiguration(FileConfigurationHandler):
    ...
    def configure(self, num_id, cassandra_ips, init_token, self_ip):
        ...
        self.cli.regex_file(
            etc_dir + '/cassandra-env.sh', 's/-Xss[1-9][0-9]*k/-Xss228k/')

        self.cli.replace_text_in_file(
            etc_dir + '/cassandra-env.sh',
            '# JVM_OPTS="$JVM_OPTS -Djava.rmi.server.hostname=<public name>"',
            'JVM_OPTS="$JVM_OPTS -Djava.rmi.server.hostname=' +
            self_ip.ip + '"')

        self.cli.regex_file_multi(
            etc_dir + '/cassandra.yaml',
