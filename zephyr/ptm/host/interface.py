...
class Interface(PTMObject):
    ...
    def __init__(self, name, host, mac=None, ip_addr=None,
                 linked_bridge=None, vlans=None):
        """
        Represents a faux-hardware link-layer interface on a host.
        This base class cannot be used to create an interface on
        the OS, but it can be used for control of a real interface
        device post-creation (a veth peer, for example).
        :type name: str
    ...
    def config_addr(self):
        if self.mac is not None:
            self.cli.cmd('ip link set dev ' + self.get_name() +
                         ' address ' + self.mac)
    ...
    def print_config(self, indent=0):
        print(('    ' * indent) + self.name + ' with ips: ' +
              ', '.join(str(ip) for ip in self.ip_list))
        if self.linked_bridge is not None:
            print(('    ' * (indent + 1)) + 'Linked on bridge: ' +
                  self.linked_bridge.name)
