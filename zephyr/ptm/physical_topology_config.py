...
# InterfaceDef := ( name, ip_addresses=IPDefList, mac_address, linked_bridge,
#                   vlans=VLANDefList )
...
#    { 'hosts'=HostDefMap, 'wiring'=WiringDefMap,
#              'implementation'=ImplementationDefMap }
...
class BridgeDef(object):
    def __init__(self, name, ip_addresses=None, mac_address=None,
                 options=None):
        """
        :type name: str
    ...
    def make_bridge(br_cfg):
        if 'name' not in br_cfg:
            raise ObjectNotFoundException(
                '"name" field required for bridge definition')
        name = br_cfg['name']
        ip_addresses = []
        mac_address = (br_cfg['mac_address']
                       if 'mac_address' in br_cfg
                       else None)
        options = (br_cfg['options'].split()
                   if 'options' in br_cfg
                   else None)
...
class VLANDef(object):
    ...
    def make_vlan(vlan_cfg):
        if 'id' not in vlan_cfg:
            raise ObjectNotFoundException(
                '"id" field required for VLAN definition')
        vid = vlan_cfg['id']
        ip_addresses = []

        for ip in (vlan_cfg['ip_addresses']
                   if 'ip_addresses' in vlan_cfg
                   else []):
            ip_addresses.append(IP(**ip))

        return VLANDef(vid, ip_addresses)
...
class InterfaceDef(object):
    ...
    def make_interface(if_cfg):
        if 'name' not in if_cfg:
            raise ObjectNotFoundException(
                '"name" field required for interface definition')
        name = if_cfg['name']
        ip_addresses = []
        mac_address = (if_cfg['mac_address']
                       if 'mac_address' in if_cfg
                       else None)
        linked_bridge = (if_cfg['linked_bridge']
                         if 'linked_bridge' in if_cfg
                         else None)
        vlans = {}
        ...
        return InterfaceDef(name, ip_addresses, mac_address,
                            linked_bridge, vlans)
...
class IPForwardRuleDef(object):
    ...
    def make_ip_forward_rule(if_cfg):
        if 'exterior' not in if_cfg:
            raise ObjectNotFoundException(
                '"exterior" field required for interface definition')
        exterior = if_cfg['exterior']
        if 'interior' not in if_cfg:
            raise ObjectNotFoundException(
                '"interior" field required for interface definition')
...
class RouteRuleDef(object):
    ...
    def make_route_rule(if_cfg):
        if 'dest' not in if_cfg:
            raise ObjectNotFoundException(
                '"dest" field required for route definition')
        dest = if_cfg['dest']
...
class HostDef(object):
    def __init__(self, name, bridges=None, interfaces=None,
                 ip_forward_rules=None, route_rules=None):
        """
        :type name: str
    ...
    def make_host(host_cfg):
        if 'name' not in host_cfg:
            raise ObjectNotFoundException(
                '"name" field required for host definition')
        name = host_cfg['name']
        bridges = {}
        interfaces = {}
        ip_forward_rules = []
...
class HostInterfaceDef(object):
    ...
    def make_host_interface(hi_cfg):
        if 'host' not in hi_cfg:
            raise ObjectNotFoundException(
                '"host" field required for host-interface definition')
        if 'interface' not in hi_cfg:
            raise ObjectNotFoundException(
                '"interface" field required for host-interface definition')
...
class WiringDef(object):
    ...
    def make_wiring(wire_cfg):
        if 'near' not in wire_cfg:
            raise ObjectNotFoundException(
                '"near" field required for wiring definition')
        if 'far' not in wire_cfg:
            raise ObjectNotFoundException(
                '"far" field required for wiring definition')
...
class ApplicationDef(object):
    ...
    def make_application(app_cfg):
        if 'class' not in app_cfg:
            raise ObjectNotFoundException(
                '"class" field required for application definition')

        return ApplicationDef(
            app_cfg['class'],
            **{k: app_cfg[k] for k in app_cfg.keys() if k != 'class'})
...
class ImplementationDef(object):
    ...
    def make_implementation(impl_cfg):
        if 'host' not in impl_cfg:
            raise ObjectNotFoundException(
                '"host" field required for implementation definition')
        if 'impl' not in impl_cfg:
            raise ObjectNotFoundException(
                '"impl" field required for implementation definition')
...
class PhysicalTopologyConfig(object):
    def __init__(self, hosts=None, wiring=None,
                 implementation=None, host_start_order=None):
        """
    ...
    def make_physical_topology(ptm_cfg):
        if 'root_server_host' not in ptm_cfg:
            raise ObjectNotFoundException(
                '"root_server_host" field required for topology definition')
        hosts = {}
        wiring = {}
        implementation = {}
        host_start_order = []
        ...
        for imp_cfg in (ptm_cfg['implementation']
                        if 'implementation' in ptm_cfg
                        else []):
            imp = ImplementationDef.make_implementation(imp_cfg)
            implementation[imp.host] = imp

        for host_name in (ptm_cfg['host_start_order']
                          if 'host_start_order' in ptm_cfg
                          else []):
            host_start_order.append(host_name)

        return PhysicalTopologyConfig(hosts, wiring,
                                      implementation, host_start_order)
