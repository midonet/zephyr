...
class TestRouterPeering(NeutronTestCase):
    ...
    def create_edge_router(self, id='1', edge_host='edge1', edge_iface='eth1',
                           edge_subnet='172.16.2.0/24'):

        # Create an uplink network (Midonet-specific extension used for provider:network_type)
        edge_network = self.api.create_network({'network': {'name': 'edge' + id,
                                                            'admin_state_up': True,
        ...
        edge_subnet = self.api.create_subnet({'subnet': {'name': 'edge_sub' + id,
        ...
        # Create edge router
        edge_router = self.api.create_router({'router': {'name': 'edge_router' + id,
                                                         'admin_state_up': True,
        ...
        edge_port1 = self.api.create_port({'port': {'name': 'edge_port' + id,
                                                    'network_id': edge_network['id'],
                                                    'admin_state_up': True,
